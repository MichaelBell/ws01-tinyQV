...
`else
        (* keep = 1 *) gf180mcu_fd_sc_mcu7t5v0__and2_1 gate_and (.A1(enable), .A2(wr), .Z(pre_reset));
        if (WIDTH < 6) begin : GEN_OR_X1
            (* keep = 1 *) gf180mcu_fd_sc_mcu7t5v0__or2_1 gate_or (.A1(pre_reset), .A2(~rst_n), .Z(gate));
        end else if (WIDTH < 12) begin : GEN_OR_X2
            (* keep = 1 *) gf180mcu_fd_sc_mcu7t5v0__or2_2 gate_or (.A1(pre_reset), .A2(~rst_n), .Z(gate));
        end else begin : GEN_OR_X4
            (* keep = 1 *) gf180mcu_fd_sc_mcu7t5v0__or2_4 gate_or (.A1(pre_reset), .A2(~rst_n), .Z(gate));
        end
        
        for (b = 0; b < WIDTH; b = b + 1)
        begin : gen_prism_bit
            (* keep = 1 *) gf180mcu_fd_sc_mcu7t5v0__latq_1 prism_cfg_bit
            (
                .D    (data_in[b]),
                .E    (gate),
                .Q    (latch_data[b])
