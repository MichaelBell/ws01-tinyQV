...
async def test_debug_reg(dut):
  ...
  for i in range(8):
    ...
    await ClockCycles(dut.clk, 1)
    for j in range(8):
        assert ((dut.uo_out.value.to_unsigned() >> 2) & 0xF) == ((val >> (4 * j)) & 0xF)
        await ClockCycles(dut.clk, 1)
    await stop_nops()

async def test_pwm(dut, pwm_value, pwm_strobe, use_ram_b):
    
    dut._log.info(f"sync to PWM rising edge")
    # sync to pwm gen
    await RisingEdge(dut.audio)
    await FallingEdge(dut.audio)
    await RisingEdge(dut.audio)
    await ClockCycles(dut.clk, 1)
    ...
    for i in range(pwm_value):
        if use_ram_b:
            ...
            assert dut.uio_out.value[7] == 1, f"RAM B should be unused"
        assert dut.audio.value == 1, f"failed on cycle {i}"
    ...
    for i in range(255 - pwm_value):
        if use_ram_b:
            ...
            assert dut.uio_out.value[7] == 1, f"RAM B should be unused"
        assert dut.audio.value == 0, f"failed on cycle {i}"
        await ClockCycles(dut.clk, pwm_strobe)
...
async def test_audio(dut):
    ...
    for pwm in (10, 61, 163, 250):
        ...
        await test_pwm(dut, pwm, 1, False)
